...
class WicImage:
    ...
    def __init__(
        self, src_wic: pathlib.Path, dest_wic: pathlib.Path, default_part: int
    ):
        self.dest_wic = dest_wic
        self.default_part = default_part
        shutil.copyfile(src_wic, self.dest_wic)

    def rm(self, path: str, part: Optional[int] = None):
        if part is None:
            part = self.default_part
        subprocess.check_output(["wic", "rm", f"{self.dest_wic}:{part}{path}"])

    def add_file(self, src: pathlib.Path, to: str, part: Optional[int] = None):
    ...
    def extract_file(self, src: str, to: pathlib.Path, part: Optional[int] = None):

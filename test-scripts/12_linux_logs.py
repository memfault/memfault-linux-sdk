...
def test_log_contains_boot_messages(
    qemu: QEMU,
    ...
    # Give a few seconds for kernel logs to be included (required)
    time.sleep(5)
    ...
    logs = memfault_service_tester.log_files_get_list(device_serial=qemu_device_id)
    log = memfault_service_tester.log_file_download(
        device_serial=qemu_device_id, cid=logs[0]["cid"]
    )

    # First kernel log message
    assert "Booting Linux on physical CPU 0x0000000000" in log, log
...
def test_shutting_down_logs_are_collected(
    qemu: QEMU,
    memfault_service_tester: MemfaultServiceTester,
    qemu_device_id: str,
):
    ...
    # Give a smidge for the log to be passed to memfaultd
    time.sleep(2)
    ...
    logs = memfault_service_tester.log_files_get_list(device_serial=qemu_device_id)
    log = memfault_service_tester.log_file_download(
        device_serial=qemu_device_id, cid=logs[-1]["cid"]
    )

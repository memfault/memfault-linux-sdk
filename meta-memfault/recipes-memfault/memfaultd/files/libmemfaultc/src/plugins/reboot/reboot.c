...
#else
  fprintf(stderr, "reboot:: Assuming we are not rebooting (systemd disabled).");
  return true;
#endif
}
    ...
    if (s_reboot_reason_sources[i].read_and_clear(memfaultd, &reason)) {
      if (!prioritized_reason_set) {
        ...
        fprintf(stderr, "reboot:: Using reboot reason %s (0x%04x) from %s source for boot_id %s\n",
                memfaultd_reboot_reason_str(reason), reason, s_reboot_reason_sources[i].name,
                boot_id);
      } else {
        fprintf(
          stderr, "reboot:: Discarded reboot reason %s (0x%04x) from %s source for boot_id %s\n",
          memfaultd_reboot_reason_str(reason), reason, s_reboot_reason_sources[i].name, boot_id);
      }
    }
  }

...
TEST_BASE(MemfaultdCollectdUtest) {
  ...
  void setup() override {
    ...
    mkdtemp(tmp_dir);
    snprintf(tmp_header_include_output_file, sizeof(tmp_header_include_output_file),
             "%s/collectd-header-include.conf", tmp_dir);
    snprintf(tmp_footer_include_output_file, sizeof(tmp_footer_include_output_file),
             "%s/collectd-footer-include.conf", tmp_dir);
  }
